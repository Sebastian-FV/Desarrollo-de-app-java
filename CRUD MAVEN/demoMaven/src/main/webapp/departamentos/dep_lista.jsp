...
<div class="container">
	...
	<tbody><c:forEach var="departamento" items="${listDep}">
		<tr>
			...
			<td><a class="btn btn-primary" href="DepartamentoServletController?action=edit&id=<c:out value="${departamento.id}"></c:out>">Editar<a/>
				<a class="btn btn-primary" href="DepartamentoServletController?action=eliminar&id=<c:out value="${departamento.id}"></c:out>">Eliminar<a/> </td>
		</tr>
		</c:forEach>
	</tbody>
</div>
